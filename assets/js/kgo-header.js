...
(function(){
  ...
  function showPharmacyRows(rows,sourceLabel){
    var items=rows.map(function(row){
      ...
      return String(row[0]||"KONYA").trim()+" · "+name+(/ECZANES[İI]$/i.test(name)?"":" ECZANESİ");
    });
    items.push("Veri: Eczane Adresi");
    setItems(pharmacyTrack,items,80);
    var districtSet=new Set();
    rows.forEach(function(row){
      var label=String(row[0]||"").toLocaleUpperCase("tr-TR");
      var central=false;
      ["SELÇUKLU","MERAM","KARATAY"].forEach(function(district){if(label.indexOf(district)>-1){districtSet.add(district);central=true;}});
      if(!central)districtSet.add(label.replace(/^\d+\. BÖLGE.*$/,"KONYA"));
    });
    if(pharmacyMeta)pharmacyMeta.textContent=todayLabel()+" · "+rows.length+" eczane · "+districtSet.size+" ilçe · "+sourceLabel;
  }
  function showPharmacyFallback(){
    var key=istanbulDateKey();
    var full=allDistrictFallback[key];
    if(Array.isArray(full)&&full.length){
      showPharmacyRows(full,"Veri: Eczane Adresi · merkez KEO ile doğrulandı");
      return;
    }
    var names=pharmacySchedule[key];
    if(Array.isArray(names)&&names.length===12){
      var rows=names.map(function(name,index){return [pharmacyAreas[index],name];});
      setItems(pharmacyTrack,rows.map(function(row){return row[0]+" · "+row[1]+" ECZANESİ";}),20);
  ...
  function loadPharmacies(){
    var endpoint="https://eczaneadresi.com/api/public/v1/duty-pharmacies?city=konya&limit=200";
    return withTimeout(endpoint,12000,{"Accept":"application/json"}).then(function(response){
      ...
      var rows=data.pharmacies.map(function(pharmacy){
        return [String(pharmacy.district||"KONYA MERKEZ").toLocaleUpperCase("tr-TR"),String(pharmacy.name||"").toLocaleUpperCase("tr-TR")];
      }).filter(function(row){return row[1].length>1;});
      if(rows.length<12)throw new Error("pharmacy-incomplete");
      rows.sort(function(a,b){return a[0].localeCompare(b[0],"tr")||a[1].localeCompare(b[1],"tr");});
      showPharmacyRows(rows,"Veri: Eczane Adresi · merkez KEO ile çapraz kontrol");
    }).catch(function(){showPharmacyFallback();});
  }
